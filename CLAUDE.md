...
| Engine build | `src/` package built (store/features/model/policy/audit/engine), CPU-only, no notebook. Found and fixed the project's most severe bug: XGBoost 3.0.0 vs the training version 3.2.0 gave a 23x different probability on identical input, silently — fixed with a hard `requirements.txt` pin + a runtime version check in `src/model.py` that fails closed on mismatch (guard independently tested: deliberately faked a mismatch, confirmed it fires). Second bug underneath it — corrupted categorical export, every category silently `-1` — root-caused to a `te` mutation, test-data only, fixed and re-exported. **Final (single-model engine): 13/13 checks pass on genuinely correct data**, `p=0.0170` for the first real, trustworthy probability this engine produced. (Later rebuilt for the 2-model ensemble — `demo_engine.py` now prints `ALL CHECKS PASSED`, 41 pytest — see the "Ensemble investigation → engine rebuild" row.) Full log: PLAN.md, `journal/`. | ✅ done |
