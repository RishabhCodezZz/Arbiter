...
| **G7 — Engine runs clean** | `decide()` works from a saved artifact, no notebook | ✅ fully passed — single-model engine: 13/13 on genuinely correct re-exported data; version-mismatch guard tested. Engine later rebuilt for the 2-model ensemble: `demo_engine.py` prints `ALL CHECKS PASSED`, both version guards tested, `pytest` 70/70 |
...
#### 🚦 G7 — Engine runs clean — ✅ **fully passed** (see §3's gate table — single-model engine 13/13; ensemble rebuild `ALL CHECKS PASSED` / 70 pytest, both version guards tested)
