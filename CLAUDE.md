...
| Engine build | `src/` package built (store/features/model/policy/audit/engine), CPU-only, no notebook. Found and fixed the project's most severe bug: XGBoost 3.0.0 vs the training version 3.2.0 gave a 23x different probability on identical input, silently — fixed with a hard `requirements.txt` pin + a runtime version check in `src/model.py` that fails closed on mismatch (guard independently tested: deliberately faked a mismatch, confirmed it fires). Second bug underneath it — corrupted categorical export, every category silently `-1` — root-caused to a `te` mutation, test-data only, fixed and re-exported. **Final (single-model engine): 13/13 checks pass on genuinely correct data**, `p=0.0170` for the first real, trustworthy probability this engine produced. (Later rebuilt for the 2-model ensemble — `demo_engine.py` now prints `ALL CHECKS PASSED`, 70 pytest — see the "Ensemble investigation → engine rebuild" row.) Full log: PLAN.md, `journal/`. | ✅ done |
...
| External code review (Codex) | An independent agent reviewed the whole repo post-migration. **Two real defects found and fixed:** (1) a train/serve skew — online `CoarseStats.std()` used population std (ddof=0) while training builds `uid_ambiguity_std_prior` with `expanding().std()` (ddof=1); `[0,2]` → `1.0` vs `1.4142` on a feature the ensemble consumes. Fixed to ddof=1 + a batch-vs-online parity test (`tests/test_store.py`). (2) a `{}`/incomplete artifact raised `KeyError` past `engine.py`'s `except ModelUnavailableError` → construction crash instead of fail-closed. Fixed: full manifest/calibrator schema validation → `ModelUnavailableError`, tested. **Also hardened:** missing `TransactionID` → clear `ValueError` at the boundary; double feature-build failure → fail closed; audit `record_hash` now covers action/probabilities/cost-params/degraded (not just the feature vector); `verify_and_replay()` replays against the record's **stored** `cost_params`, not current module constants; `dashboard.html`/`docket.html` dropped the Google Fonts `@import` so "offline/self-contained" is literally true. **Disclosed, not fixed** (production-grade, out of scope for a solo build): keyed/chained audit signatures (exception item 12), concurrency-safe feature store (item 13), versioned artifact release + golden vectors (item 14). `pytest` 70/70, `demo_engine.py` `ALL CHECKS PASSED`. Full detail: `docs/eval_report.md` exception list, `journal/`. | ✅ done |
