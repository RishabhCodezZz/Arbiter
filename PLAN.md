...
| Cost model | V5 calibration, cost model, policy bands | ✅ **done — G6 passed. Shipped 2-model ensemble: lift +₹1.678cr vs no system / +₹77.03L vs naive 0.5** (single-XGBoost cost-model run was +₹1.54cr / +₹64.3L) |
| Engine build | Decision engine + audit trail | ✅ **done — re-export clean, engine verified (`ALL CHECKS PASSED`), version guard tested and fires correctly. Later rebuilt for the 2-model ensemble — see the "Engine rebuild" row.** |
...
| Robustness checks | Bootstrap CI + fair rules-baseline on the real full test month | ✅ **done — `scripts/robustness_checks.py`, re-run for the shipped 2-model ensemble.** Lift vs no system: 95% CI ₹1.510cr–₹1.850cr (point +₹1.678cr). Lift vs naive 0.5: ₹64.9L–₹89.5L (+₹77.03L). Both clear of zero — not a lucky month. Best rules-only threshold (₹512,535) never blocks anything in this data, collapsing to the "no system" value — fraud isn't amount-separable here, the full lift is real model signal. `tests/` (pytest, 98 passing) also closes the `demo_fallbacks.py` open item formally. **Extended a second time:** (1) the false-positive-cost estimate (~172, aggregate-curve proxy) replaced with an exact per-transaction count from the real 3-way policy — 223 real false positives of 1,314 total blocks, exact cost ₹13.20 lakh (single-XGBoost baseline: 233 / ₹17.97L); (2) PR-AUC/ROC-AUC given the same 95% bootstrap CI already given to the rupee lift (0.5439–0.5771 / 0.9070–0.9180), hand-rolled and verified against the published point estimates before being trusted. Honest exception list dropped from 8 items to 7 (item 5, now resolved, removed rather than left stale). See CLAUDE.md §13, `docs/eval_report.md` §2b/§4, `journal/`. |
| Error analysis + val/test gap decomposition | Manual error analysis on real FPs/FNs (clue skill ch14) + training-dev gap decomposition (ch40/41) | ✅ **done — real results in.** Gap decomposition: variance (train′→training-dev) **−0.151 PR-AUC**, temporal mismatch (training-dev→val) **−0.225 PR-AUC — the dominant gap**, further mismatch (val→test) −0.061. Confirms the val/test drop is overwhelmingly temporal mismatch, not overfitting — a controlled measurement of the project's own "unseen clients" thesis, not just a citation of it. Error analysis (233 exact FPs, 100 sampled FNs): errors cluster exactly where base fraud rates predict — FP 81.1% in ProductCD='C' (11.7% base rate), FN 75% in ProductCD='W' (2.0% base rate); 0 of 100 false negatives were anywhere near the operating threshold, so threshold-tuning has a verified **zero** ceiling on that population. **Decision: not retraining** — see decision log below. Sensitivity-map lift-over-baselines gap (self-flagged since the cost-model stage) closed in the same pass: fully local, confirmed positive across the entire 35-point grid (single-model min ₹1.09cr vs no system / ₹57.4L vs naive; shipped ensemble min +₹1.163cr / +₹66.77L). Full numbers: `docs/eval_report.md` §8, `docs/experiments.md`, `journal/`. |
| Hyperparameter sweep — revisiting "not retraining" | Bounded, evidence-gated check: is the shipped model's low regularization an artifact of tuning against an inflated (temporally-close-to-train) validation signal? | ✅ **done — hypothesis not supported.** Pre-committed rule selected the most conservative config ("all four combined"); checked once on test it scored **0.5290 PR-AUC vs. the then-shipped single model's 0.5514 (−0.0224, a real regression)**. Model unchanged. Reinforces, doesn't reverse, the original not-retraining decision — now backed by a direct negative test, not just reasoning. Full numbers: `docs/experiments.md`, `journal/`. |
| Segment-aware calibration — acting on the error-analysis finding | Does per-ProductCD Platt calibration (vs. one global calibrator) reduce the 233-FP cluster in ProductCD='C' (81.1% of FPs, `eval_report.md` §8b — single-model diagnostic; the shipped ensemble has 223 FPs total)? Calibration-only, no retraining. | ✅ **done — real partial win, net rejected.** All 5 segments got their own calibrator. ProductCD='C' false positives dropped 26.8% (val, 127→93) and 22.8% (test, 189→146) — the scoped hypothesis was correct. But total realized value dropped too (−0.18% val, −12.39L/−0.72% test) because recalibrating the other 4 segments cost more than 'C' saved. Pre-committed rule (adopt only if value doesn't drop) correctly says `keep_global`, confirmed on test. **Not adopted.** A C-only variant is a possible future test, not pursued — effect size (<1% of headline value) doesn't clear the bar for another round trip right now. Full numbers: `docs/experiments.md`. |
...
| Ensemble diagnostic — LightGBM + CatBoost averaged with the shipped XGBoost | Does averaging with 2 more model families close any of the gap to the Kaggle-winning ensemble (0.9408 ROC-AUC)? User explicitly willing to ship a real 3-model engine if the number justifies it. | ✅ **confirmed real — the first "make it better" win this session.** 3-model ensemble: test PR-AUC 0.5628 vs the then-shipped single XGBoost's 0.5514 (+0.0114). Paired bootstrap (2,000 resamples): 95% CI **[+0.0081, +0.0146] — excludes zero.** 2-model (XGB+LGB only) also real: +0.0083, CI [+0.0059, +0.0109] — **this is what shipped.** Unlike the sweep and segment calibration, this clears the bar the user set for a real ship conversation. Secondary finding: untuned LightGBM alone has both the best individual test score AND the smallest val→test degradation (−7.3% relative vs XGBoost's −10.0%) — a second, independent hint that Optuna's val-tuning may chase val's temporal-proximity bias, consistent with the hyperparameter sweep's own finding. CatBoost is the weakest individually and degrades *most* val→test, yet still adds incremental ensemble value (diversity, not mismatch-robustness). **Decision now live:** 2-model vs 3-model for an eventual ship, and the scope/timing of the engine rebuild (multi-model loading, SHAP across models, re-verifying every downstream artifact). Full numbers: `docs/experiments.md`, `journal/`. |
...
*(none currently — all three items previously tracked here are resolved: notebook 01 runs clean (V0 baseline, UID investigation — see §4), the cost curve has a confirmed interior minimum (G6, §3 — p=0.774 for the single model, p=0.589 for the shipped 2-model ensemble), and the LLM-as-classifier benchmark completed (G9, §3/§4). Left visibly empty rather than deleted, so it's clear these were tracked and closed, not simply forgotten about.)*
...
| **G6 — Cost curve has interior min** | Optimum is not at t=0 or t=1 | ✅ passed — minimum at p=0.774 (single model), robust to the FX correction; p=0.589 for the shipped 2-model ensemble, still interior |
| **G7 — Engine runs clean** | `decide()` works from a saved artifact, no notebook | ✅ fully passed — single-model engine: 13/13 on genuinely correct re-exported data; version-mismatch guard tested. Engine later rebuilt for the 2-model ensemble: `demo_engine.py` prints `ALL CHECKS PASSED`, both version guards tested, `pytest` 98/98 |
...
**Actual:** minimum at p=0.774 (single model; p=0.589 for the shipped 2-model ensemble), well inside [0,1]. Confirmed robust to a real bug-hunt: caught a stale-placeholder FX rate (USD→INR=83, uncorroborated) before the FIRST run even mattered, then corrected it to a live dated quote (95.41) after the first run and re-verified the gate still passes — same result, slightly stronger. Headline (shipped 2-model ensemble): Arbiter values the test month at ₹17.355cr vs ₹15.68cr (no system) and ₹16.585cr (naive 0.5 threshold). **Lift: +₹1.678cr vs no system, +₹77.03L vs naive** (single-XGBoost cost-model run was ₹17.22cr / +₹1.54cr / +₹64.3L). Of Arbiter's total, 100.16% is hard-computed from actual labels (allow/block) — the modeled step-up component is slightly negative (−₹2.78L) for the ensemble's policy — so the headline claim is entirely evidence-based. Full numbers: `docs/experiments.md`.

**Closed.** Extended via `scripts/robustness_checks.py` (Part 3) rather than left open: the *lift over both baselines* — not just Arbiter's own value — is now confirmed positive across the **entire** 35-point margin×fee grid, not just the assumed 20%/₹500 point. Minimum lift vs no system across the whole grid: +₹1.163cr for the shipped 2-model ensemble (₹1.09cr for the single-XGBoost run), still comfortably positive at the least favorable corner. Minimum lift vs naive 0.5: +₹66.77L (was ₹57.4L). Full grid: `docs/robustness_results.json`.
...
#### 🚦 G7 — Engine runs clean — ✅ **fully passed** (see §3's gate table — single-model engine 13/13; ensemble rebuild `ALL CHECKS PASSED` / 98 pytest, both version guards tested)
