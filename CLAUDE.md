...
**Result, on the untouched test month (92,427 transactions), with Razorpay's real MDR and a real dated FX rate:**

| Policy | Total value |
|---|---|
| No fraud system | ₹15.68 crore |
| Naive 0.5 threshold | ₹16.58 crore |
| **Arbiter** | **₹17.22 crore** |

**Lift: +₹1.54 crore vs no system, +₹64.3 lakh vs the industry-default naive threshold.** Policy mix: 95.8% allow, 2.7% step-up, 1.5% block. Of Arbiter's total value, **99.95% comes from allow/block outcomes computed from actual fraud labels** — hard, verifiable — only 0.05% depends on the modeled step-up population rates. The headline claim is overwhelmingly evidence-based, not assumption-dependent. Full numbers, sensitivity table: `docs/experiments.md`.

**False-positive cost, exactly — not estimated.** Of the 1,348 blocks, 233 are real genuine customers wrongly blocked (1,115 are correctly-blocked real fraud) — an exact per-transaction count from `artifacts/test_month_raw.json`, not the aggregate-curve estimate (~172) reported earlier in the project. Exact cost: **₹17.97 lakh** (₹7,712 average per wrongly-blocked customer). Both PR-AUC (0.5514) and ROC-AUC (0.9077) now also carry a 95% bootstrap CI (0.5350–0.5688 and 0.9019–0.9132) — the same rigor already given to the rupee lift, extended to the model-quality metrics themselves. Full numbers: `docs/robustness_results.json`, `docs/eval_report.md` §1/§4.
...
| Cost model | Run. **G6 passed** (interior minimum, p=0.774). **Lift: +₹1.54cr vs no system, +₹64.3L vs naive 0.5**, 99.95% hard-verified. Caught & fixed an unverified FX placeholder in the same pass as the MDR addition. See §6. ~~Time-to-detection metric~~ *(cut with component C)*. | ✅ done |
| Engine build | `src/` package built (store/features/model/policy/audit/engine), CPU-only, no notebook. Found and fixed the project's most severe bug: XGBoost 3.0.0 vs the training version 3.2.0 gave a 23x different probability on identical input, silently — fixed with a hard `requirements.txt` pin + a runtime version check in `src/model.py` that fails closed on mismatch (guard independently tested: deliberately faked a mismatch, confirmed it fires). Second bug underneath it — corrupted categorical export, every category silently `-1` — root-caused to a `te` mutation, test-data only, fixed and re-exported. **Final: 13/13 checks pass on genuinely correct data**, `p=0.0170` for the first real, trustworthy probability this engine has produced. Full log: PLAN.md, `journal/`. | ✅ done |
