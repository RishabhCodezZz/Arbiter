...
**Not for scoring.** 590k rows of tabular features is gradient-boosting territory: faster, cheaper, more accurate. **Proven, not just claimed — `scripts/llm_benchmark.py`.** On a 200-row held-out sample: XGBoost PR-AUC 0.5735 vs `gpt-oss:20b` (a real, capable model given a fair shot, not a strawman) PR-AUC 0.1571 — **3.65x more accurate**, and ~100x faster per call (~70ms per transaction end-to-end on plain CPU vs a 6.7s median LLM latency — the LLM ran on Kaggle's own GPU after the free Ollama Cloud tier proved unreliable under sustained load — see `journal/`). (The ~70ms is mostly a one-row pandas build + the sklearn wrapper; the trees themselves score in under a millisecond.) That benchmark is the answer to *"where you chose not to use one."*
